/*
 * sdc_probe_workload_d12.c — D12组 (D11跨循环ACE + D10广覆盖 + D8 lsu forwarding) gem5工作负载
 *
 * D10: bit=8.0%(=B) struct=17.0%(极显著) — 广覆盖+全volatile
 * D11: bit=8.8%(>B!) struct=10.6%(不显著) — 4跨循环高ACE
 * → 合并: D11的4跨循环ACE + D10的16操作数广覆盖 + 多路径 + lsu forwarding
 *
 * Build: gcc -static -O2 -o sdc_probe_workload_d12 sdc_probe_workload_d12.c
 */
#include <stdio.h>
#include <stdint.h>
#define ITERS 200
...
/* carry_chain: 8操作数(D10广覆盖) + 全volatile + 多引用 */
