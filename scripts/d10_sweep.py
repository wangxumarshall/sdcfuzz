...
WL=os.path.expanduser("~/gem5-fi/smoke_test/sdc_probe/sdc_probe_workload_d10")
GOLDEN="SUM=12153838441679149476 CRC=720d1fd4"
NC=78948; ROI_LO=int(NC*0.2); ROI_HI=int(NC*0.8)
def sweep(mode, n_runs, seed, structural=False):
    rng=random.Random(seed)
    rd=os.path.expanduser(f"~/gem5-fi/smoke_test/d10_{mode}_runs")
    ...
    tot=c+e+m+n
    print(f"D10 {mode}: total={tot} clean={c} ({100*c/tot:.1f}%) exit={e} masked={m}")
