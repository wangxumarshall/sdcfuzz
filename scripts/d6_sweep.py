...
"""d6_sweep.py — D6(操作数多次引用) bit-flip + 结构 500次 sweep"""
import os, random, shutil, subprocess, re
...
WL=os.path.expanduser("~/gem5-fi/smoke_test/sdc_probe/sdc_probe_workload_d6")
GOLDEN="SUM=3574323982909424612 CRC=0d23a213"
NC=69564; ROI_LO=int(NC*0.2); ROI_HI=int(NC*0.8)

def sweep(mode, n_runs, seed, structural=False):
    rng=random.Random(seed)
    rd=os.path.expanduser(f"~/gem5-fi/smoke_test/d6_{mode}_runs")
    ...
    for i in range(n_runs):
        ...
        os.makedirs(out)
        cmd=[GEM5,"-r","-e","--silent-redirect","-d",out,SCRIPT,
             "--binary",WL,"--mode","inject","--first-clock",str(fc),
             "--max-faults","1","--probability","1.0","--rng-seed",str(seed+i)]
        if structural:
    ...
    tot=c+e+m+n
    print(f"D6 {mode}: total={tot} clean={c} ({100*c/tot:.1f}%) exit={e} masked={m}")

import sys
mode=sys.argv[1] if len(sys.argv)>1 else "bit"
n=int(sys.argv[2]) if len(sys.argv)>2 else 500
s=int(sys.argv[3]) if len(sys.argv)>3 else 42
if mode=="struct": sweep("struct",n,s,structural=True)
else: sweep("bit",n,s)
