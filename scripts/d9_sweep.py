...
WL=os.path.expanduser("~/gem5-fi/smoke_test/sdc_probe/sdc_probe_workload_d9")
...
def sweep(mode, n_runs, seed, structural=False):
    ...
    rd=os.path.expanduser(f"~/gem5-fi/smoke_test/d9_{mode}_runs")
    ...
    tot=c+e+m+n
    print(f"D9 {mode}: total={tot} clean={c} ({100*c/tot:.1f}%) exit={e} masked={m}")
