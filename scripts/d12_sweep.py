...
WL=os.path.expanduser("~/gem5-fi/smoke_test/sdc_probe/sdc_probe_workload_d12")
GOLDEN="SUM=17591145999354937338 CRC=950f9fd4"
NC=80399; ROI_LO=int(NC*0.2); ROI_HI=int(NC*0.8)
def sweep(mode, n_runs, seed, structural=False):
    rng=random.Random(seed)
    rd=os.path.expanduser(f"~/gem5-fi/smoke_test/d12_{mode}_runs")
    ...
    tot=c+e+m+n
    print(f"D12 {mode}: total={tot} clean={c} ({100*c/tot:.1f}%) exit={e} masked={m}")
