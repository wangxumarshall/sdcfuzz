...
Silent Data Corruption (SDC) on commercial ARM server CPUs is increasingly reported at fleet scale, yet the dominant detection norm—random coverage-guided fuzzing of a CPU proxy followed by fleet-scale end-state replay—operates in the *instruction-coverage* space and treats all operands uniformly. We argue and empirically show that this leaves a detection gap: the *operand-combinatorial* space, which controls which microarchitectural gates toggle and which carry-chain paths activate, is the more sensitive axis. We present a dictionary-guided operand-space mutation methodology that, parameterized against published TaiShan V110 microarchitecture, produces a 175-snapshot stress corpus targeting seven weak modules (MMU/L2C/LSU/OoO/IEX/FSU/IFU). We deploy this corpus at near-full-load across a 4-board, 446-core Kunpeng 920 fleet and contribute a load-noise taxonomy that distinguishes resource-exhaustion runaway (RunSnapOutcome=5) and signal-misbehave (=6) from genuine SDC (outcome ∈ {2,3,4})—without which the 1606 full-load runaways on one board would be misreported as 1606 SDC events. We validate fault-sensitivity in a gem5 V110 O3 model: the operand-dictated workload yields a 4.3% single-bit-injection diverge rate (18/417) and 8.0% multi-bit (max-faults=3, 4/50). A pre-registered A/B comparison against a structurally-identical random-operand workload yields a **falsification**: the operand-dictated diverge rate (4.3%) is *lower* than the random-operand rate (9.6%), A/B ratio 0.45×—pre-registered threshold (≥2× for significance) not met. We honestly report this as a calibration: operand-dictated targeting does not improve bit-flip-injection fault sensitivity (likely due to logical masking of structured extreme-value results), and the operand-targeting thesis holds only conjecturally for real-silicon structural-defect excitation. Across 446 cores over a 24-hour scan (interim, ongoing), zero genuine SDC was observed—consistent with the expected 10⁻⁸–10⁻¹⁰ SDC rate on healthy silicon, and consistent with the gap between model-level fault sensitivity and silicon-level defect detection that we articulate as the open problem. This work does not reproduce the known core-179 structural SDC (Paper 1 establishes bit-flip injectors cannot reach it); it positions silifuzz instruction-pressure deployment as a complementary real-silicon excitation axis whose structural-defect reach is conjectural and unverified.
...
3. **A/B operand-dictated vs random-operand fault sensitivity (pre-registered, falsified)** (§5.2). Two structurally-identical gem5 workloads (same functions, same ITERS, same instruction topology) differing only in operand choice—A: operand-dictated extreme values; B: LCG-random operands—under identical single-bit and multi-bit gem5 injection. Pre-registered threshold: A≥2×B = significant. **Result: A=4.3% (18/417), B=9.6% (29/303), A/B=0.45× — falsified.** The operand-dictated rate is *lower* than random. We honestly report this as a calibration: operand-targeting does not improve bit-flip fault sensitivity (logical masking hypothesis, §5.2), and the operand-targeting thesis narrows to conjectural real-silicon structural-defect excitation (§6).
...
**A results (completed)**: 500 injections (417 effective, single 200s timeout interrupted run_457), 18 clean diverge (SUM/CRC differ from golden, no gem5 exit), 2 gem5-exit diverge. **A single-bit clean diverge rate: 4.3%** (18/417). Most sensitive registers: integer[9] (5 hits), [12]/[1]/[7] (3 each)—all high-use in the carry-chain/toggle paths.
...
**B results (pre-registered A/B, completed)**: B workload (random operands) under identical single-bit injection: 303 effective runs (of 500, single 200s timeout interrupted some), **29 clean diverge, 9.6% rate**. Most sensitive registers: integer[1]/[0] (2 each), [2]/[4]/[27] (1 each)—broader distribution than A.

**A/B ratio = 0.45× (A=4.3%, B=9.6%)** — A (operand-dictated) is *lower* than B (random). Pre-registered threshold: A≥2×B → significant; 1.5-2× → marginal; <1.5× → no significant difference. **The result is below 1×: the operand-targeting thesis is FALSIFIED for the bit-flip-injection fault-sensitivity metric.**

**Honest interpretation**: this is a negative result that *strengthens* the paper's honesty and is scientifically interesting. We hypothesize the mechanism: operand-dictated extreme values (0xFFFF...+1 → deterministic 0; 0x5555^0xAAAA → deterministic all-ones) produce *deterministic, structured* results where bit-flips may be logically masked (e.g., a flipped bit in a carry-chain operand still yields a result that CRC catches only probabilistically); random operands produce *unstructured* results where bit-flips more often yield observable divergence because there is no structured redundancy to mask against. This is consistent with the masking literature [CITE TBD: Relyzer masking, verify]. The honest conclusion: **operand-dictated targeting does not improve bit-flip fault sensitivity in the gem5 model; the operand-targeting thesis holds only for real-silicon structural-defect excitation (conjectural, §6), not for bit-flip-injection sensitivity.** We report this as a calibration of where operand-targeting does and does not help—a more honest and useful finding than a naive "operand-targeting is better" claim.
...
The honest central tension. Paper 1 establishes that the core-179 SDC is a *structural* fault (byte-lane skew in the load data-return path) and that bit-flip injection cannot reproduce it in simulation. Our gem5 validation (§5.2) uses bit-flip injection—by the same limitation, it cannot reach structural defects. The A/B falsification (A=4.3% vs B=9.6%) sharpens this: even within the bit-flip-injection metric, operand-dictated targeting is *not* more sensitive than random—so the 4.3% is a calibration floor, not a superiority claim. What the operand-dictated corpus proves is only that it is *fault-sensitive to bit-flip-class faults* in the V110 O3 model (and less so than random, due to masking). It does **not** prove detection of structural defects.
...
We presented a dictionary-guided operand-space mutation methodology for constructing SDC stress corpora on closed-RTL commercial ARM server silicon, parameterized against published TaiShan V110 microarchitecture, producing a 175-snapshot corpus deployed at 446-core fleet scale. We contributed a full-load noise taxonomy that prevents 1606 runaways from being misreported as SDC. The pre-registered A/B comparison **falsified** the operand-targeting-improves-bit-flip-sensitivity thesis (A=4.3% vs B=9.6%, ratio 0.45×)—an honest negative result that calibrates where operand-targeting does and does not help: it does not improve bit-flip-injection fault sensitivity (logical masking of structured extreme-value results), and the operand-targeting thesis narrows to conjectural real-silicon structural-defect excitation. Zero genuine SDC was observed on healthy silicon—a negative result we frame as honest calibration, not failure. The gap between model-level fault sensitivity and silicon-level structural-defect detection is articulated as the open problem, with Paper 1's structural fault injector as the simulation-side complement and real-silicon instruction-pressure excitation as the conjectural, unverified bridge.
