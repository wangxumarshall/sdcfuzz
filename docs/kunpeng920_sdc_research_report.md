...
| **★★★★** | **D10 bit=B持平(8.0%) + struct极显著超B(17.0%, 2.02x, p<0.001)** | D10(全volatile+多引用+16操作数覆盖广) bit-flip=8.0%(40/500)=B=8.0%持平! 结构故障=17.0%(85/500)>B=8.4%(42/500), 2.02x, z=4.08, p=0.000044极显著。**两度量都不低于B! bit持平+struct极显著超B = 击败SiliFuzz!** |
...
| **★★★★★★** | **D13完整: bit=24.6%(3.07x极显著) + struct=65.4%(7.79x极显著) 碾压SiliFuzz!** | D13(随机+定向变异)完整500次: bit-flip=24.6%(123/500) vs B=8.0%(3.07x, z=7.11, p<0.001极显著), 结构=65.4%(327/500) vs B=8.4%(7.79x, z=18.68, p<0.001极显著)。**两度量都极显著碾压SiliFuzz!** 用户洞察'基于随机值的定向变异'完全验证。 |
