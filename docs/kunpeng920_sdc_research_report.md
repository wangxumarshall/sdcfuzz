...
研究建立了从"激发（gem5-fi）→ 检出（silifuzz）→ 真机部署（分布式扫描）"的证据链，证明检测用例端到端有效。
...
| **★★★★** | **D10 bit=B持平(8.0%) + struct极显著超B(17.0%, 2.02x, p<0.001)** | D10(全volatile+多引用+16操作数覆盖广) bit-flip=8.0%(40/500)=B=8.0%持平, 结构故障=17.0%(85/500)>B=8.4%(42/500), 2.02x, z=4.08, p=0.000044极显著。**两度量都不低于B：bit 持平，struct 极显著超B。** |
| **★★★★★** | **D12 bit显著超B(12.4%,1.55x,p=0.02) + D13 bit极显著超B(19.7%,2.46x,p<0.001)** | D12(D11跨循环ACE+D10广覆盖+D8 forwarding) bit=12.4%显著超B(1.55x,p=0.02), struct=14.8%极显著超B(1.76x,p=0.002)。D13(D12+随机+定向变异选择) bit=19.7%(中途142)极显著超B(2.46x,p<0.001) — 用户洞察'基于随机值的定向变异'验证。 |
| **★★★★★★** | **D13完整: bit=24.6%(3.07x极显著) + struct=65.4%(7.79x极显著)，两度量均极显著优于B** | D13(随机+定向变异)完整500次: bit-flip=24.6%(123/500) vs B=8.0%(3.07x, z=7.11, p<0.001极显著), 结构=65.4%(327/500) vs B=8.4%(7.79x, z=18.68, p<0.001极显著)。**两度量均极显著优于B。** 用户洞察'基于随机值的定向变异'得到验证。 |
